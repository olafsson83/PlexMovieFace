...
class ProvenSurvivalTests(unittest.TestCase):
    def test_proven_track_survives_long_gap_and_resumes_on_keep(self):
        mgr = manager()
        ...
        # Weak re-detection (0.47 in blur): >= keep resumes swapping via the
        # KEEP bar instead of failing cold acquisition.
        out = mgr.observe([FakeFace({"1": 0.47})])
        self.assertEqual(len(out), 1)

    def test_unproven_track_still_dies_at_the_short_limit(self):
        mgr = manager()
        mgr.observe([FakeFace({"1": 0.9})])  # accepted, but only 1 score
        for _ in range(3):
            mgr.observe([])
